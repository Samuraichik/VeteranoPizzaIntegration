...
class PromoViewController: UIViewController {
    ...
    let foodArr = VeteranoInfo.shared.getPromoArr()
    ...
    private lazy var foodTableView: UITableView = {
        ...
        PromoTableViewCell.register(for: tableView)
        return tableView
    }()
...
extension PromoViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = PromoTableViewCell.deque(for: tableView, indexPath: indexPath) else {
            return UITableViewCell()
        }
        let selectedFood = foodArr[indexPath.row]
        cell.buttonAction = { [self] in
            let ingredientsViewController = PromoIngredientsViewController()
